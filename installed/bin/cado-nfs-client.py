...
def detect_installed_tree(pathdict):
    ...
    if bd:
        if os.environ.get("CADO_NFS_DEBUG_PATHDETECT"):
            print("{} does not end in bin".format(mydir))
        return False
    example = os.path.join(install_tree, "lib/cado-nfs-3.0.0", one_pyfile_example_subpath)
    ...
    pathdict["pylib"] = os.path.join(install_tree, "lib/cado-nfs-3.0.0/scripts/cadofactor")
    pathdict["data"]  = os.path.join(install_tree, "share/cado-nfs-3.0.0")
    pathdict["lib"]   = os.path.join(install_tree, "lib/cado-nfs-3.0.0")
    pathdict["bin"]   = os.path.join(install_tree, "bin")
